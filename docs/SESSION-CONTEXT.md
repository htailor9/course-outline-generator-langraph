...
- **Duplicate-name requirement implemented** (STUDIOPE-446 class): scan found duplicate lesson names within units in 7 outlines (standards-43, 600–1000). Fixed: `uniquify_chapter_names` now runs across the whole unit (not per packing bucket) and is casefold-aware; new `uniquify_part_names` guarantees unique unit names; new soft `NAMES` invariant; 3 regression tests. Module-in-lesson uniqueness was already enforced.
  - 2026-09-08 strengthening: the spec (`course_outline_requirements.md` §2.6, STUDIOPE-446) permits disambiguation "using subtopic descriptors (e.g. `Solving Ratio Problems - Equivalent Ratios`) **or** numerical suffixes (e.g. `Order Of Operations Mastery (2)`)". We now implement the descriptor method exclusively in practice — differentiator searches skill words, then the objective's own text — and a numeric suffix can only occur for word-for-word identical objectives (synthetic clones), i.e. stricter than the spec's permitted option (`29b65ee`).
...
- Codebase formatted with **black**; committed as `b377aef` (69 tests at that point).
- **Context-based unit regeneration built** (STUDIOPE-94): `python -m outline regenerate <run-dir> --unit <N|name> [--prompt ...]`. `outline/regen.py` rebuilds state deterministically from the prior run's `input.json`+`outline.json`; non-target units locked (placements/lesson names/titles reused); target unit re-planned + re-titled with the previous version as prompt context and user-prompt priority; whole course re-flows through pack/assemble/validate; output = new run folder + `regeneration.md` before/after diff; baseline untouched (= undo). Tests prove non-target units byte-stable and target changed. **Full-course regeneration** also built: `--unit all` re-runs the whole graph with the previous outline's unit/lesson structure summarised into every planning prompt (`_regen_context` → `course_header`), `--prompt` priority on top; note lists units before/after. **Lesson-level regeneration** also built (`--unit N --lesson M`): re-titles ONE lesson's modules, placements + all other titles byte-locked; intro/apply/review/test/semester lessons excluded per STUDIOPE-94. **Prompt guard** built (`outline/prompt_guard.py`, STUDIOPE-286/335/338): injection/override/unrelated prompts rejected with user-facing messages before any LLM call, wired into both `generate` and `regenerate`. Regeneration use-cases live-tested: full+context+prompt ✅, full+context no-prompt ✅ (one transient claude_cli timeout → deterministic fallback, still valid), unit+prompt ✅ (31/31 non-target modules byte-identical), unit no-prompt ✅; lesson-level + guard proven offline. Suite now **82 passed, 1 skipped**. Remaining out of scope: LG-level regeneration, slates, Studio save/undo DB lifecycle (ACTIVE/SUPERSEDED, 409 handling), §3.2 prompt-intent live test pack. Undo/redo UI and full-history versioning still out of scope.
