...
def test_llm_schemas_roundtrip():
    a = AnnotateOut.model_validate(
        {"items": [{"id": "L1", "verb": "identify", "primary_skill": "Main Idea"}]}
    )
    assert a.items[0].id == "L1"
    p = PartsOut.model_validate(
        {"parts": [{"part_name": "Logic", "ids": ["L1", "L2"]}]}
    )
    assert p.parts[0].ids == ["L1", "L2"]
    c = ChaptersOut.model_validate(
        {"assignments": [{"id": "L1", "chapter_name": "Fallacies", "order_rank": 1}]}
    )
    assert c.assignments[0].order_rank == 1
    t = TitlesOut.model_validate(
        {"modules": [{"id": "L1", "title": "Logical Fallacy Identification"}]}
    )
...
def test_outline_accepts_golden(golden43):
    ...
    assert (
        o.children[1].children[1].children[0].learning_objective_urn.startswith("urn:")
    )
