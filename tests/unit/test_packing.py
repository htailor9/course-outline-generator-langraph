...
def test_splits_on_time_limit():
    chapters = pack_chapters(
        [lo(100, 30), lo(100, 30), lo(100, 30)], word_limit=2000, minute_limit=60
    )
...
def test_splits_on_density_4():
    chapters = pack_chapters(
        [lo(10, 1) for _ in range(9)], word_limit=2000, minute_limit=60
    )
...
def test_module_numbers_reset_per_chapter():
    chapters = pack_chapters(
        [lo(10, 1) for _ in range(5)], word_limit=2000, minute_limit=60
    )
    assert [m["module_number"] for m in chapters[0]["learning_objectives"]] == [
        1,
        2,
        3,
        4,
...
def test_chapter_name_from_sources():
    chapters = pack_chapters(
        [lo(10, 1, "A"), lo(10, 1, "B")], word_limit=2000, minute_limit=60
    )
